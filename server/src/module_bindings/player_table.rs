...
/// Access to the `username_canonical` unique index on the table `player`,
/// which allows point queries on the field of the same name
/// via the [`PlayerUsernameCanonicalUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().username_canonical().find(...)`.
pub struct PlayerUsernameCanonicalUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `username_canonical` unique index on the table `player`.
    pub fn username_canonical(&self) -> PlayerUsernameCanonicalUnique<'ctx> {
        PlayerUsernameCanonicalUnique {
            imp: self
                .imp
                .get_unique_constraint::<String>("username_canonical"),
            phantom: std::marker::PhantomData,
        }
    }
}

impl<'ctx> PlayerUsernameCanonicalUnique<'ctx> {
    /// Find the subscribed row whose `username_canonical` column value is equal to `col_val`,
    /// if such a row is present in the client cache.
...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    ...
    _table.add_unique_constraint::<String>("username_canonical", |row| &row.username_canonical);
