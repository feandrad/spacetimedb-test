// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#nullable enable

using System;
using SpacetimeDB.ClientApi;
using System.Collections.Generic;
using System.Runtime.Serialization;

namespace SpacetimeDB.Types
{
    public sealed partial class RemoteReducers : RemoteBase
    {
        public delegate void ListResourcesByTypeHandler(ReducerEventContext ctx, string resourceType);
        public event ListResourcesByTypeHandler? OnListResourcesByType;

        public void ListResourcesByType(string resourceType)
        {
            conn.InternalCallReducer(new Reducer.ListResourcesByType(resourceType), this.SetCallReducerFlags.ListResourcesByTypeFlags);
        }

        public bool InvokeListResourcesByType(ReducerEventContext ctx, Reducer.ListResourcesByType args)
        {
            if (OnListResourcesByType == null)
            {
                if (InternalOnUnhandledReducerError != null)
                {
                    switch (ctx.Event.Status)
                    {
                        case Status.Failed(var reason): InternalOnUnhandledReducerError(ctx, new Exception(reason)); break;
                        case Status.OutOfEnergy(var _): InternalOnUnhandledReducerError(ctx, new Exception("out of energy")); break;
                    }
                }
                return false;
            }
            OnListResourcesByType(
                ctx,
                args.ResourceType
            );
            return true;
        }
    }

    public abstract partial class Reducer
    {
        [SpacetimeDB.Type]
        [DataContract]
        public sealed partial class ListResourcesByType : Reducer, IReducerArgs
        {
            [DataMember(Name = "resource_type")]
            public string ResourceType;

            public ListResourcesByType(string ResourceType)
            {
                this.ResourceType = ResourceType;
            }

            public ListResourcesByType()
            {
                this.ResourceType = "";
            }

            string IReducerArgs.ReducerName => "list_resources_by_type";
        }
    }

    public sealed partial class SetReducerFlags
    {
        internal CallReducerFlags ListResourcesByTypeFlags;
        public void ListResourcesByType(CallReducerFlags flags) => ListResourcesByTypeFlags = flags;
    }
}
