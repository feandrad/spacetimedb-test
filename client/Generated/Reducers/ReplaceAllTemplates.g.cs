// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#nullable enable

using System;
using SpacetimeDB.ClientApi;
using System.Collections.Generic;
using System.Runtime.Serialization;

namespace SpacetimeDB.Types
{
    public sealed partial class RemoteReducers : RemoteBase
    {
        public delegate void ReplaceAllTemplatesHandler(ReducerEventContext ctx, System.Collections.Generic.List<SpacetimeDB.Types.MapTemplate> newTemplates);
        public event ReplaceAllTemplatesHandler? OnReplaceAllTemplates;

        public void ReplaceAllTemplates(System.Collections.Generic.List<SpacetimeDB.Types.MapTemplate> newTemplates)
        {
            conn.InternalCallReducer(new Reducer.ReplaceAllTemplates(newTemplates), this.SetCallReducerFlags.ReplaceAllTemplatesFlags);
        }

        public bool InvokeReplaceAllTemplates(ReducerEventContext ctx, Reducer.ReplaceAllTemplates args)
        {
            if (OnReplaceAllTemplates == null)
            {
                if (InternalOnUnhandledReducerError != null)
                {
                    switch (ctx.Event.Status)
                    {
                        case Status.Failed(var reason): InternalOnUnhandledReducerError(ctx, new Exception(reason)); break;
                        case Status.OutOfEnergy(var _): InternalOnUnhandledReducerError(ctx, new Exception("out of energy")); break;
                    }
                }
                return false;
            }
            OnReplaceAllTemplates(
                ctx,
                args.NewTemplates
            );
            return true;
        }
    }

    public abstract partial class Reducer
    {
        [SpacetimeDB.Type]
        [DataContract]
        public sealed partial class ReplaceAllTemplates : Reducer, IReducerArgs
        {
            [DataMember(Name = "new_templates")]
            public System.Collections.Generic.List<MapTemplate> NewTemplates;

            public ReplaceAllTemplates(System.Collections.Generic.List<MapTemplate> NewTemplates)
            {
                this.NewTemplates = NewTemplates;
            }

            public ReplaceAllTemplates()
            {
                this.NewTemplates = new();
            }

            string IReducerArgs.ReducerName => "replace_all_templates";
        }
    }

    public sealed partial class SetReducerFlags
    {
        internal CallReducerFlags ReplaceAllTemplatesFlags;
        public void ReplaceAllTemplates(CallReducerFlags flags) => ReplaceAllTemplatesFlags = flags;
    }
}
