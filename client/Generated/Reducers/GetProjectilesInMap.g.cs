// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#nullable enable

using System;
using SpacetimeDB.ClientApi;
using System.Collections.Generic;
using System.Runtime.Serialization;

namespace SpacetimeDB.Types
{
    public sealed partial class RemoteReducers : RemoteBase
    {
        public delegate void GetProjectilesInMapHandler(ReducerEventContext ctx, string mapId);
        public event GetProjectilesInMapHandler? OnGetProjectilesInMap;

        public void GetProjectilesInMap(string mapId)
        {
            conn.InternalCallReducer(new Reducer.GetProjectilesInMap(mapId), this.SetCallReducerFlags.GetProjectilesInMapFlags);
        }

        public bool InvokeGetProjectilesInMap(ReducerEventContext ctx, Reducer.GetProjectilesInMap args)
        {
            if (OnGetProjectilesInMap == null)
            {
                if (InternalOnUnhandledReducerError != null)
                {
                    switch (ctx.Event.Status)
                    {
                        case Status.Failed(var reason): InternalOnUnhandledReducerError(ctx, new Exception(reason)); break;
                        case Status.OutOfEnergy(var _): InternalOnUnhandledReducerError(ctx, new Exception("out of energy")); break;
                    }
                }
                return false;
            }
            OnGetProjectilesInMap(
                ctx,
                args.MapId
            );
            return true;
        }
    }

    public abstract partial class Reducer
    {
        [SpacetimeDB.Type]
        [DataContract]
        public sealed partial class GetProjectilesInMap : Reducer, IReducerArgs
        {
            [DataMember(Name = "map_id")]
            public string MapId;

            public GetProjectilesInMap(string MapId)
            {
                this.MapId = MapId;
            }

            public GetProjectilesInMap()
            {
                this.MapId = "";
            }

            string IReducerArgs.ReducerName => "get_projectiles_in_map";
        }
    }

    public sealed partial class SetReducerFlags
    {
        internal CallReducerFlags GetProjectilesInMapFlags;
        public void GetProjectilesInMap(CallReducerFlags flags) => GetProjectilesInMapFlags = flags;
    }
}
